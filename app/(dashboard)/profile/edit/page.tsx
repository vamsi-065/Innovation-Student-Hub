  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-zinc-400 text-sm">Update your public profile information</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="glass-card p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1.5">Profile Picture URL</label>
              <input name="avatar" value={form.avatar} onChange={handleChange} placeholder="https://..." className="input-glass" id="edit-avatar" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-zinc-400 mb-1.5">University</label>
              <input name="university" value={form.university} onChange={handleChange} placeholder="MIT" className="input-glass" id="edit-university" />
            </div>
            <div>
              <label className="block text-xs text-zinc-400 mb-1.5">Department</label>
              <input name="department" value={form.department} onChange={handleChange} placeholder="CS" className="input-glass" id="edit-department" />
            </div>
            <div>
              <label className="block text-xs text-zinc-400 mb-1.5">Year</label>
              <input name="year" value={form.year} onChange={handleChange} placeholder="3" className="input-glass" id="edit-year" />
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-6">
          <h2 className="text-sm font-semibold text-zinc-300 mb-4">Skills</h2>
            ...
            <div className="flex flex-wrap gap-1.5">
              {SKILL_SUGGESTIONS.filter((s) => !skills.includes(s)).slice(0, 8).map((s) => (
                <button
                  ...
                  type="button"
                  onClick={() => addSkill(s)}
                  className="text-xs px-3 py-1 rounded-full border border-white/8 text-zinc-500 hover:text-zinc-300 hover:border-white/15 transition-all"
                >
                  + {s}
                </button>
              ))}
        ...
        {/* Social Links */}
        <div className="glass-card p-6 space-y-4">
          <h2 className="text-sm font-semibold text-zinc-300 mb-4">Social Links</h2>
          {[
